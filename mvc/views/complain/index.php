            ...
            <div class="col-sm-12">
                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody>
                            <?php if(customCompute($complains)) {$i = 1; foreach($complains as $complain) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('complain_title')?>">
                                        <?php
                                            if (strlen((string) $complain->title) > 30) {
    echo strip_tags(substr((string) $complain->title, 0, 30)."...");
} else {
    echo strip_tags(substr((string) $complain->title, 0, 30));
}
                                        ?>
                                    ...
                                    <td data-title="<?=$this->lang->line('complain_description')?>">
                                        <?php
                                            if (strlen((string) $complain->description) > 30) {
                                            echo strip_tags(substr((string) $complain->description, 0, 30)."...");
                                        } else {
                                            echo strip_tags(substr((string) $complain->description, 0, 30));
                                        }
                                        ?>
