<?php if (customCompute($applicant)) { ?>
	<div class="well">
		<div class="row">
			<div class="col-sm-6">
		...
		<div class="row">
					...
					<div class="box-body box-profile">
						...
						<ul class="list-group list-group-unbordered">
							...
							<?php } else { ?>
								...
								<li class="list-group-item" style="background-color: #FFF">
									<b><?= $this->lang->line('leaveapplication_dob') ?></b> <a class="pull-right"><?= date('d M Y', strtotime((string) $applicant->dob)) ?></a>
								</li>
								<li class="list-group-item" style="background-color: #FFF">
									<b><?= $this->lang->line('leaveapplication_phone') ?></b> <a class="pull-right"><?= $applicant->phone ?></a>
								</li>
			...
			<div class="col-sm-6 col-mergin">
				<div class="box box-primary box-height">
					<div class="box-body box-profile">
						<h5><?= $this->lang->line('leaveapplication_date') ?> - <?= date('d M Y', strtotime((string) $leaveapplication->apply_date)); ?></h5>
				...
				<div class="box box-primary">
					<div class="box-body box-profile">
						<div class="single_box">
							...
							<p class="text-muted"><?= date("d M Y", strtotime((string) $leaveapplication->from_date)); ?> - <?= date("d M Y", strtotime((string) $leaveapplication->to_date)); ?></p>
						</div>
						<div class="single_box">
						...
						<div class="single_box">
							<strong><?= $this->lang->line('leaveapplication_attachment') ?></strong>
							<p class="text-muted">
								<?php
						...
						<?php if (($siteinfos->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) { ?>
							<div class="single_box">
								<?php if ($leaveapplication->status == null) { ?>
									<button class="btn btn-succes leave-status" data-status="1" data-id="<?= $leaveapplication->leaveapplicationID ?>" data-placement="top" data-toggle="tooltip" data-original-title="Approve" style="background: #5CB85C !important; color:#FFF !important"><?= $this->lang->line('leaveapplication_status_approve') ?></button>
									<button class="btn btn-danger leave-status" data-status="0" data-id="<?= $leaveapplication->leaveapplicationID ?>" data-placement="top" data-toggle="tooltip" data-original-title="Decline"><?= $this->lang->line('leaveapplication_status_decline') ?></button>
									<?php } else {
									if ($leaveapplication->status == 1) {
									?>
										<a class="btn btn-success btn-block" style="background: #5CB85C !important; color:#FFF !important;cursor:auto"><?= $this->lang->line('leaveapplication_status_approve') ?></a>
									<?php
									} else {
									?>
										<a class="btn btn-danger btn-block" style="cursor:auto"><?= $this->lang->line('leaveapplication_status_decline') ?></a>
									<?php
									}
									?>
