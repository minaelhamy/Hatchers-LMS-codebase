                    ...
                    <div id="hide-table">
                        <table id="withoutBtn" class="table table-hover dataTable no-footer">
                            ...
                            <?php if ($conversations): ?>
                                <?php if(customCompute($conversations)) {$i = 1; foreach($conversations as $conversation) { ?>
                                    <tr class="">
                                        ...
                                        <td data-title="<?= $this->lang->line('subject') ?>" class="mailbox-subject"><a href='<?= base_url("conversation/view/$conversation->id") ?>'><b><?= substr((string) $conversation->subject, 0, 10) . ".." ?></b> </a></td>
                                        <td data-title="<?= $this->lang->line('attach') ?>" class="mailbox-attachment"><?php if ( $conversation->attach != '' ) { ?><i class="fa fa-paperclip"></i><?php } ?></td>
                                        <td data-title="<?= $this->lang->line('time') ?>" class="mailbox-date"><?=date('d M Y h:i:s A', strtotime((string) $conversation->create_date))?></td>
                                        <td data-title="<?= $this->lang->line('action') ?>" class="mailbox-reply"><a href="<?= base_url('conversation/draft_send/' . $conversation->id); ?>" class="btn btn-xs btn-primary"><i class="fa fa-paper-plane" aria-hidden="true"></i><?=$this->lang->line('sent'); ?></a></td>
                                    </tr>
                                <?php $i++; }} ?>    
                            <?php endif ?>
                            </tbody>
