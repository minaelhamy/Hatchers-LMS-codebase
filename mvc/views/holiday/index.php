            ...
            <div class="col-sm-12">
                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody>
                            <?php if(customCompute($holidays)) {$i = 1; foreach($holidays as $holiday) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('holiday_title')?>">
                                        <?php
                                            if (strlen((string) $holiday->title) > 25) {
    echo strip_tags(substr((string) $holiday->title, 0, 25)."...");
} else {
    echo strip_tags(substr((string) $holiday->title, 0, 25));
}
                                        ?>
                                    ...
                                    <td data-title="<?=$this->lang->line('holiday_fdate')?>">
                                        <?php echo date("d M Y", strtotime((string) $holiday->fdate)); ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('holiday_tdate')?>">
                                        <?php echo date("d M Y", strtotime((string) $holiday->tdate)); ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('holiday_details')?>">
                                        <?php
                                            if (strlen((string) $holiday->details) > 60) {
                                            echo strip_tags(substr((string) $holiday->details, 0, 60)."...");
                                        } else {
                                            echo strip_tags(substr((string) $holiday->details, 0, 60));
                                        }
                                        ?>
