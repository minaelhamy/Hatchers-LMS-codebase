                            ...
                            <?php if(customCompute($notices)) {$i = 1; foreach($notices as $notice) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('notice_title')?>">
                                        <?php 
                                            if (strlen((string) $notice->title) > 25) {
                                                echo strip_tags(substr((string) $notice->title, 0, 25)."...");
                                            } else {
                                                echo strip_tags(substr((string) $notice->title, 0, 25));
                                            }
                                        ?>
                                    ...
                                    <td data-title="<?=$this->lang->line('notice_date')?>">
                                        <?php echo date("d M Y", strtotime((string) $notice->date)); ?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('notice_notice')?>">
                                        <?php 
                                            if (strlen((string) $notice->notice) > 60) {
                                                echo strip_tags(substr((string) $notice->notice, 0, 60)."...");
                                            } else {
                                                echo strip_tags(substr((string) $notice->notice, 0, 60));
                                            }
                                        ?>
