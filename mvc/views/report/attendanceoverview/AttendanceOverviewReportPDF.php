...
<body>
    ...
    $monthdays = $monthID;
    $monthday = explode('-',(string) $monthdays);
?>
    <!-- form start -->
    <div class="col-sm-12">
        <?=reportheader($siteinfos, $schoolyearsessionobj, true)?>
    </div>
    <h3 class="box-title text-navy"> <?=$this->lang->line('attendanceoverviewreport_reportfor')?> <?=$this->lang->line('attendanceoverviewreport_type')?> - <?=$attendanceoverviewreport_reportfor?> ( <?php if($usertype ==1 && $siteinfos->attendance == 'subject' && isset($subjects[$subjectID])) { echo $subjects[$subjectID]." , "; } ?> <?=$monthArray[$monthday[0]].' '.$monthday[1]?> ) </h3>
    <?php if($usertype == '1') { ?>
        <div class="col-sm-12">
