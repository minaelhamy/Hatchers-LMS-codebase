<div class="row">
    <div class="col-sm-12" style="margin:10px 0px">
        <?php
            if($fromdate != '' && $todate != '') {
                $pdf_preview_uri = base_url('duefeesreport/pdf/'.$classesID.'/'.$sectionID.'/'.$studentID.'/'.$feetypeID.'/'.strtotime((string) $fromdate).'/'.strtotime((string) $todate));
            } else {
                $pdf_preview_uri = base_url('duefeesreport/pdf/'.$classesID.'/'.$sectionID.'/'.$studentID.'/'.$feetypeID);
            }

            if($fromdate != '' && $todate != '') {
                $xml_preview_uri = base_url('duefeesreport/xlsx/'.$classesID.'/'.$sectionID.'/'.$studentID.'/'.$feetypeID.'/'.strtotime((string) $fromdate).'/'.strtotime((string) $todate));
            } else {
                                    ...
                                    <?php $totalDue = 0; $i = 0; foreach($getDueFeesReports as $getDueFeesReport) { 
                                        if($sectionID > 0) { if(isset($students[$getDueFeesReport->studentID]) && $students[$getDueFeesReport->studentID]->srsectionID == $sectionID) { $i++; ?>
                                            <tr>
                                                ...
                                                <td data-title="<?=$this->lang->line('duefeesreport_invoice_date')?>"><?=date('d M Y',strtotime((string) $getDueFeesReport->create_date))?></td>
