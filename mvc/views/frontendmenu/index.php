<div class="nav-tabs-custom">
    ...
    <div class="tab-content">
        <div id="edit-menus" class="tab-pane active">
        	<div class="row">
        		<div class="col-sm-12">
	        		<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data" <?=customCompute($fmenus) ? '' : 'style="display: none"' ?>>
    ...
    foreach ($fmenus as $fmenu) { ?>
					...
					<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data" <?=customCompute($fmenus) ? 'style="display: none"' : '' ?>>
	        			<div class="form-group">
							<label class="col-sm-12 control-label top-menu-color" style="cursor: text">
        	...
        	<div class="row">
				<div class="col-sm-4">
					<div class="panel-group <?=customCompute($fmenus) ? '' : 'disable-menu' ?>" id="menu-settings" aria-multiselectable="true">
									...
									<div class="tab-content">
									    <div class="tab-pane active" id="most-recent">
											<div class="menu-settings-header with-border pages-all-checkbox">
												<?php if($pages) { foreach ($pages as $page) { if($page->status == 1) { ?>
													<div class="form-group">
														...
														<label for="page-<?=$page->pagesID?>"><?=namesorting($page->title, 35)?></label>
													</div>
												<?php } } } ?>
											</div>
											<div class="menu-settings-footer clearfix">
												...
												<?php } ?>
												<input <?=customCompute($fmenus) ? '' : 'disabled' ?> type="button" class="submit-btn pull-right pages-add" value="<?=$this->lang->line('frontendmenu_add_to_menu')?>">
											</div>
									    </div>
							      	</div>
							    </div>
											...
											<div class="menu-settings-header with-border posts-all-checkbox">
												<?php if($posts) { foreach ($posts as $post) { if($post->status == 1) { ?>
													<div class="form-group">
														...
														<label for="post-<?=$post->postsID?>"><?=namesorting($post->title, 35)?></label>
													</div>
												<?php } } } ?>
											</div>
											<div class="menu-settings-footer clearfix">
												...
												<?php } ?>
												<input <?=customCompute($fmenus) ? '' : 'disabled' ?> type="button" class="submit-btn pull-right posts-add" value="<?=$this->lang->line('frontendmenu_add_to_menu')?>">
											</div>
									    </div>
							      	</div>
							    </div>
						...
						<div class="panel panel-default">
						    ...
						    <div id="menu-settings-collapse-links" class="panel-collapse collapse" role="tabpanel" aria-labelledby="page-heading-links">
								<div class="panel-body">
									...
									<div class="menu-settings-footer clearfix">
										<input <?=customCompute($fmenus) ? '' : 'disabled' ?> type="button" class="submit-btn pull-right links-add" value="<?=$this->lang->line('frontendmenu_add_to_menu')?>">
									</div>
								</div>
						    </div>
						</div>
				...
				<div class="col-sm-8">
					<div id="menu-management">
						<div id="create-new-menu-box" <?=customCompute($fmenus) ? 'style="display: none"' : '' ?>>
						...
						<div id="menu-manage-box" <?=customCompute($fmenus) ? '' : 'style="display: none"' ?>>
							<div class="menu-management-header">
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
									...
									<div class="col-sm-6">
										<div class="publishing-menu-action pull-right">
											<input type="button" <?=customCompute($fmenus) ? '' : 'disabled' ?> name="save-name" class="save-menu submit-menu" value="<?=$this->lang->line('frontendmenu_save_menu')?>" >
										</div>
									</div>
								</div>
							</div>
							<div id="menu-post-body">
								...
								<div class="menu-settings">
									...
									<fieldset class="menu-settings-list">
									    ...
									    <div class="menu-settings-input">
									        <input <?=customCompute($activefmenu) ? ($activefmenu->topbar == 1) ? ' checked="checked" ' : '' : ''?> type="checkbox"  name="menu-locations_top" id="locations-top" value="1">
									        <label for="locations-top"><?=$this->lang->line('frontendmenu_top_menu')?></label>
									    </div>
									    <div class="menu-settings-input">
									        <input <?=customCompute($activefmenu) ? ($activefmenu->social == 1) ? ' checked="checked" ' : '' : ''?>  type="checkbox" name="menu_locations_social" id="locations-social" value="1">
								...
								<div class="menu-management-footer clearfix">
									...
									<input <?=customCompute($fmenus) ? '' : 'disabled' ?> type="button"  class="save-menu pull-right submit-menu" value="<?=$this->lang->line('frontendmenu_save_menu')?>">
								</div>
							</div>
						</div>
					</div>
        ...
        <div id="manage-locations" class="tab-pane"> 
        	...
        	<form class="form-horizontal" role="form" method="post" enctype="multipart/form-data" action="<?=base_url('frontendmenu/managelocation')?>">
                ...
                <div>
                	<button <?=customCompute($fmenus) ? '' : 'disabled' ?> type="submit" class="btn save-menu"><?=$this->lang->line('frontendmenu_save_changes')?></button>
                </div>
            </form>
        </div>
    </div>
...
<script type="text/javascript">
	...
	$(document).on('click', '.submit-menu', function() {
		var activeMenuID = "<?=$getactivefmenuID?>";
		var loop = 1;
		var arr = [];
		var flagArray = [];
	   	...
	   	if(arr.length > 0) {
			...
			var editmenuID = $('#topbar-menu-select-property').val();
			var menuname = $('#menu-name').val();

			$.ajax({
	            type: 'POST',
	            url: "<?=base_url('frontendmenu/savemenu')?>",
	            data: { 'elements' : arr, 'locationtop' : locationtop, 'locationsocial' : locationsocial, 'editmenuID' : editmenuID, 'menuname' : menuname },
	            dataType: "html",
	            success: function(data) {
	...
	$(document).on('click', '.btn-menu-add-type', function() {
		var fmenuID = $('#topbar-menu-select-property').val();
		if(fmenuID != '') {
			$.ajax({
	            type: 'POST',
	            url: "<?=base_url('frontendmenu/editmenutoggle')?>",
	            data: { 'fmenuID' : fmenuID },
	            dataType: "html",
	            success: function(data) {
	            	window.location.reload();
	            }
	...
	$(document).on('click', '.delete-btn', function(e) {
		e.preventDefault();
		var fmenuID = "<?=$getactivefmenuID?>";
		if(fmenuID != '') {
			$.ajax({
	            type: 'POST',
	            url: "<?=base_url('frontendmenu/deletefmenu')?>",
	            data: { 'fmenuID' : fmenuID },
	            dataType: "html",
	            success: function(data) {
	            	window.location.reload();
	...
	$(document).on('click', '.submit-create-new-menu', function() {
		var createmenuname = $('#create-menu-name').val();
		if(createmenuname != '') {
			$.ajax({
	            type: 'POST',
	            url: "<?=base_url('frontendmenu/createnewmenu')?>",
	            data: { 'menuname' : createmenuname },
	            dataType: "html",
	            success: function(data) {
	            	var response = JSON.parse(data);
	            	if (response.status == false) {
