...
<div id="printablediv">
    <section class="panel">
        <div class="profile-view-head-cover" style="background-image: url(<?=base_url('uploads/images/'.$event->photo)?>);">
          <h1 class="img-thumbnail picture-left"><?=date("d M", strtotime((string) $event->fdate))?></h1>
          <?php if($event->fdate != $event->tdate) { ?>
          <h1 class="img-thumbnail picture-right"><?=date("d M", strtotime((string) $event->tdate))?></h1>
          <?php } ?>
        </div>
        ...
        <div class="text-center">
          <div class="btn-group">
              ...
              <button style="background-color:#0A75AF" class="btn btn-facebook" data-toggle="modal" data-target="#goings" type="button"><?=customCompute($goings)?></button>
          </div>
          <div class="btn-group">
              <button style="background-color:#B00303" class="btn btn-bitbucket" data-toggle="modal" data-target="#ignores"  type="button"><?=customCompute($ignores)?></button>
              <button id="ignore" style="background-color:#C23321" class="btn btn-facebook" type="button"><?=$this->lang->line('interested')?></button>
          </div>
        </div>
        <br/>
        <div class="panel-body profile-view-dis">
          <div class="text-center">
            <h1><?=$event->title?></h1>
            <h4>
              <?=date("d M Y", strtotime((string) $event->fdate))?>
              <?php echo " at ".date("h:i A", strtotime((string) $event->ftime));
                if($event->fdate != $event->tdate) {
                  echo " <b>to</b> ".date("d M Y", strtotime((string) $event->tdate))." at ";
                } else {
                  echo " - ";
                }
                echo date("h:i A", strtotime((string) $event->ttime));
              ?>
            </h4>
          </div>
          <br/>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title"><?=$this->lang->line('going')?> (<?=customCompute($goings)?>)</h4>
        </div>
        <div class="modal-body">
          <table class="table table-bordered">
            ...
            <?php if(customCompute($goings)) { $i=1; foreach ($goings as $going) { ?>
              <thead>
                <tr>
                  <td><?=$i?></td>
...
<div class="modal fade" id="ignores">
  <div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
            <h4 class="modal-title"><?=$this->lang->line('interested')?> (<?=customCompute($ignores)?>)</h4>
        </div>
        <div class="modal-body">
          <table class="table table-bordered">
            <thead>
            ...
            <?php if(customCompute($ignores)) { $i=1; foreach ($ignores as $ignore) { ?>
              <thead>
                <tr>
                  <td><?=$i?></td>
