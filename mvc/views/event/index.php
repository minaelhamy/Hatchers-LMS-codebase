                                ...
                                foreach ($events as $event) { ?>
                                    <tr>
                                        ...
                                        <td data-title="<?= $this->lang->line('event_title') ?>">
                                            <?php
                                            if (strlen((string) $event->title) > 25) {
                                                echo strip_tags(substr((string) $event->title, 0, 25) . "...");
                                            } else {
                                                echo strip_tags(substr((string) $event->title, 0, 25));
                                            }
                                            ?>
                                        ...
                                        <td data-title="<?= $this->lang->line('event_fdate') ?>">
                                            <?php echo date("d M Y", strtotime((string) $event->fdate)) . " (" . date("h:i A", strtotime((string) $event->ftime)) . ")"; ?>
                                        </td>
                                        <td data-title="<?= $this->lang->line('event_tdate') ?>">
                                            <?php echo date("d M Y", strtotime((string) $event->tdate)) . " (" . date("h:i A", strtotime((string) $event->ttime)) . ")"; ?>
                                        </td>
                                        <td data-title="<?= $this->lang->line('event_details') ?>">
                                            <?php
                                            if (strlen((string) $event->details) > 60) {
                                                echo strip_tags(substr((string) $event->details, 0, 60) . "...");
                                            } else {
                                                echo strip_tags(substr((string) $event->details, 0, 60));
                                            }
                                            ?>
