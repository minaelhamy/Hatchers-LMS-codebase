            ...
            <div class="col-sm-12">
                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody>
                            <?php if(customCompute($instructions)) {$i = 1; foreach($instructions as $instruction) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('instruction_title')?>">
                                        <?php
                                            if (strlen((string) $instruction->title) > 25) {
                    echo strip_tags(substr((string) $instruction->title, 0, 25)."...");
                } else {
                    echo strip_tags(substr((string) $instruction->title, 0, 25));
                }
                                        ?>
                                    ...
                                    <td data-title="<?=$this->lang->line('instruction_content')?>">
                                        <?php
                                            if (strlen((string) $instruction->content) > 150) {
                                            echo substr((string) $instruction->content, 0, 150)."...";
                                        } else {
