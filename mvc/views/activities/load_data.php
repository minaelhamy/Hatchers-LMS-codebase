        ...
        <div class="box box-widget" style="margin-bottom: 25px;">
            <div class="box-header with-border social-media">
                <div class="user-block">
                    ...
                    <span class="description"><?=$this->lang->line('activities_shared_publicly')?> - <?php echo date("l jS \of M Y h:i:s A", strtotime((string) $activity->create_date));?></span>
                </div>
                <div class="box-tools">
            ...
            <div class="box-body">
                <div class="container-fluid no-padding">
                    <div class="row">
                        <?php $status = TRUE; $k = 0; if(isset($activitiesmedia[$activity->activitiesID])) { foreach ($activitiesmedia[$activity->activitiesID] as $attachment) { ?>
                            <?php if(customCompute($activitiesmedia[$activity->activitiesID]) == 2) { $status = FALSE; ?>
                                ...
                                <?php $k++; } elseif(customCompute($activitiesmedia[$activity->activitiesID]) > 1) { if($status) { ?>
                ...
                <?php if($activity->time_from) { ?>
                    <button type="button" class="btn btn-info btn-xs"><i class="fa fa-clock-o"></i> <?php echo date("h:i:s A", strtotime((string) $activity->time_from));?></button>
                <?php } ?>
                <?php if($activity->time_to) { ?>
                    <button type="button" class="btn btn-info btn-xs"><i class="fa fa-clock-o"></i> <?php echo date("h:i:s A", strtotime((string) $activity->time_to));?></button>
                <?php } ?>
                <?php if($activity->time_at) { ?>
                    <button type="button" class="btn btn-info btn-xs"><i class="fa fa-clock-o"></i> <?php echo date("h:i:s A", strtotime((string) $activity->time_at));?></button>
                <?php } ?>
                <span class="pull-right text-muted">
                        <?php
                        if(isset($activitiescomments[$activity->activitiesID])) {
                            if(customCompute($activitiescomments[$activity->activitiesID]) == 1) {
                                echo customCompute($activitiescomments[$activity->activitiesID]).' '.$this->lang->line('activities_comment');
                            } else {
                                echo customCompute($activitiescomments[$activity->activitiesID]).' '.$this->lang->line('activities_comments');
                            }
                        }
                        ?>
                    </span>
