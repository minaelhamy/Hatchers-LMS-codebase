            ...
            <div class="col-sm-12">
                ...
                <div id="hide-table">
                    <table id="example1" class="table table-striped table-bordered table-hover dataTable no-footer">
                        ...
                        <tbody>
                            <?php if(customCompute($pagess)) {$i = 1; foreach($pagess as $pages) { ?>
                                <tr>
                                    ...
                                    <td data-title="<?=$this->lang->line('pages_title')?>">
                                        ...
                                        <?php if($pages->status == 2 || $pages->status == 4) {
                                                echo '<b>--'. ucfirst((string) pageStatus($pages->status, FALSE)).'</b>';
                                            }
                                        ?>
                                        <?php if(strtotime((string) $pages->publish_date) > strtotime(date('Y-m-d H:i:s'))) {
                                    ...
                                    <td data-title="<?=$this->lang->line('pages_template')?>">
                                        <?=ucfirst((string) $pages->template)?>
                                    </td>
                                    <td data-title="<?=$this->lang->line('pages_date')?>">
                                        <?=date('d M Y h:i A', strtotime((string) $pages->publish_date))?>
                                    </td>
                                  <?php if(permissionChecker('pages_edit') || permissionChecker('pages_delete')) { ?>
