...
abstract class REST_Controller extends CI_Controller {
    ...
    {   
        ...
        $this->_supported_formats = ['json' => 'application/json', 'xml' => 'application/xml', 'html' => 'text/html'];
        ...
        if ($this->auth_override === FALSE &&
            (! ($this->config->item('rest_enable_keys') && $this->_allow === TRUE) ||
                ($this->config->item('allow_auth_and_keys') === TRUE && $this->_allow)))
        {
    ...
    protected function preflight_checks()
    {
        ...
        if (explode('.', (string) CI_VERSION, 2)[0] < 3)
        {
		...
		if(!$isProfilingEnabled){
        	...
        	elseif ($data !== NULL)
        	{
            	...
            	if (method_exists(Format::class, 'to_' . $this->response->format))
            	{
                	...
                	$this->output->set_content_type($this->_supported_formats[$this->response->format], strtolower((string) $this->config->item('charset')));
    ...
    protected function _detect_input_format()
    {
        ...
        if (empty($content_type) === FALSE)
        {
            ...
            $content_type = (strpos((string) $content_type, ';') !== FALSE ? current(explode(';', (string) $content_type)) : $content_type);
    ...
    protected function _detect_output_format()
    {
        ...
        if (preg_match($pattern, (string) $this->uri->uri_string(), $matches))
        {
            return $matches[1];
        }
        ...
        {
            $format = strtolower((string) $this->_get_args['format']);
        ...
        if ($this->config->item('rest_ignore_http_accept') === FALSE && $http_accept !== NULL)
        {
            ...
            foreach (array_keys($this->_supported_formats) as $format)
            {
                ...
                if (strpos((string) $http_accept, $format) !== FALSE)
                {
                    ...
                    elseif ($format === 'html' && strpos((string) $http_accept, 'xml') === FALSE)
                    {
                        // HTML or XML have shown up as a match
                        // If it is truly HTML, it wont want any XML
                        return $format;
                    } elseif ($format === 'xml' && strpos((string) $http_accept, 'html') === FALSE) {
    ...
    protected function _detect_api_key()
    {
        ...
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', (string) $api_key_variable));
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            ...
            if (empty($row->is_private_key) === FALSE)
            {
                ...
                if (isset($row->ip_addresses))
                {
                    ...
                    $list_ip_addresses = explode(',', (string) $row->ip_addresses);
    ...
    protected function _check_limit($controller_method)
    {
        ...
        switch ($this->config->item('rest_limits_method'))
        {
            ...
            default:
                ...
                if (strpos(strrev((string) $limited_uri), strrev((string) $this->response->format)) === 0)
                {
                    $limited_uri = substr((string) $limited_uri,0, -strlen((string) $this->response->format) - 1);
                }
    ...
    protected function _parse_put()
    {
        if ($this->request->format) {
            ...
            {
                $this->_put_args = json_decode((string) $this->input->raw_input_stream);
            }
    ...
    protected function _parse_head()
    {
        ...
        parse_str(parse_url((string) $this->input->server('REQUEST_URI'), PHP_URL_QUERY), $head);
    ...
    protected function _parse_options()
    {
        ...
        parse_str(parse_url((string) $this->input->server('REQUEST_URI'), PHP_URL_QUERY), $options);
    ...
    protected function _perform_library_auth($username = '', $password = NULL)
    {
        ...
        $auth_library_class = strtolower((string) $this->config->item('auth_library_class'));
        $auth_library_function = strtolower((string) $this->config->item('auth_library_function'));

        if ($auth_library_class === '')
        {
            log_message('debug', 'Library Auth: Failure, empty auth_library_class');
            return FALSE;
        }

        if ($auth_library_function === '')
        {
    ...
    protected function _check_login($username = NULL, $password = FALSE)
    {
        ...
        $auth_source = strtolower((string) $this->config->item('auth_source'));
        $rest_auth = strtolower((string) $this->config->item('rest_auth'));
        ...
        return $valid_logins[$username] === $password;
    }
    ...
    protected function _prepare_basic_auth()
    {
        ...
        elseif ($http_auth !== NULL)
        {
            ...
            if (stripos((string) $http_auth, 'basic') === 0)
            {
                // Search online for HTTP_AUTHORIZATION workaround to explain what this is doing
                list($username, $password) = explode(':', base64_decode(substr((string) $this->input->server('HTTP_AUTHORIZATION'), 6)));
            }
        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $matches = [];
        preg_match_all('@(username|nonce|uri|nc|cnonce|qop|response)=[\'"]?([^\'",]+)@', (string) $digest_string, $matches);
        ...
        $md5 = md5(strtoupper((string) $this->request->method).':'.$digest['uri']);
        ...
        if (strcasecmp((string) $digest['response'], $valid_response) !== 0)
        {
            // Display an error response
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
    ...
    protected function _check_blacklist_auth()
    {
        ...
        if (preg_match($pattern, (string) $this->config->item('rest_ip_blacklist')))
        {
            // Display an error response
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
    ...
    {
        $whitelist = explode(',', (string) $this->config->item('rest_ip_whitelist'));
        $whitelist[] = '127.0.0.1';
        $whitelist[] = '0.0.0.0';
    ...
    protected function _check_access()
    {
        ...
        return !empty($accessRow) && !empty($accessRow['all_access']);
    }
    ...
    public function jwt_encode($token)
    {
