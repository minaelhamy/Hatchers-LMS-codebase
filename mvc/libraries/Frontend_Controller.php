...
class Frontend_Controller extends MY_Controller
{
    ...
    */
    ...
    private string $_frontendTheme;
    private string $_frontendThemePath;
    private string $_frontendThemeBasePath;
    ...
    function __construct()
    {
        ...
        if(frontendData::get_backend('frontendorbackend') == 'NO'){
        ...
        /* Start All Data Call */
        $this->data['loadMoreBtn'] = false;
        $this->data['latestevents'] = $this->event_m->get_latest_events(6,['schoolyearID' => $schoolyearID]);
        $this->data['events'] = $this->event_m->get_events_with_limit_offset(0, 7,['schoolyearID' => $schoolyearID]);
        $this->data['teachers'] = $this->teacher_m->general_get_order_by_teacher(['active' => 1]);
        $this->data['notices'] = $this->notice_m->get_notice_with_limit_offset(0, 7,array('schoolyearID' => $schoolyearID));
        $this->data['sociallink'] = pluck_multi_array_key($this->sociallink_m->get_sociallink(), 'obj', 'usertypeID', 'userID');
        $this->data['classes'] = $this->classes_m->general_get_classes();
        $this->data['allcountry'] = $this->getCountrys();
        /* Close All Data Call */
        $this->data['homepage'] = $this->pages_m->get_one($this->data['frontend_topbar']);

        $this->data['all_pages']      = $this->pages_m->get_pages();
        ...
        $this->_frontendTheme = strtolower((string) $this->data["backend_setting"]->frontend_theme);
        ...
        $this->bladeView->set('loadMoreBtn', $this->data['loadMoreBtn']);
        $this->bladeView->set('all_pages', $this->data['all_pages']);
        ...
        $this->bladeView->set('allcountry', $this->data['allcountry']);
    ...
    private function callMenu()
    {
        ...
        if (customCompute($this->data['frontend_topbar'])) {
            ...
            } elseif (customCompute($this->data['frontend_social'])) {
        ...
        return array('frontendTopbarMenus' => $frontendTopbarMenu, 'frontendSocialMenus' => $frontendSocialMenu, 'frontendTopbarQueryMenus' => $frontendTopbarQueryMenus, 'frontendSocialQueryMenus' => $frontendSocialQueryMenus);
    }
        ...
        if (customCompute($elements)) {
            ...
            if (customCompute($elements)) {
                ...
                foreach ($elements as $elementKey => $element) {
                    if (customCompute($mergeElements)) {
                        foreach ($mergeElements as $mergeElementKey =>  $mergeElement) {
                            if (isset($mergeElement['child']) && customCompute($mergeElement['child'])) {
