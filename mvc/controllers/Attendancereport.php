...
*/
	...
	public function getAttendacneReport() {
		...
		if(permissionChecker('attendancereport')) {
			if($_POST !== []) {
				...
				$date		    = explode('-',(string) $this->input->post('date'));
	...
	public function pdf() {
		if(permissionChecker('attendancereport')) {		
			$attendancetype = htmlentities((string) escapeString($this->uri->segment(3)));
			...
			if($this->data["siteinfos"]->attendance == 'subject') {
				...
				$dateString = date('d-m-Y',htmlentities((string) escapeString($this->uri->segment(7))));
				$date  = explode('-', $dateString);
				$flag = FALSE;
			} else{
				$dateString = date('d-m-Y',htmlentities((string) escapeString($this->uri->segment(6))));
				$date  = explode('-', $dateString);
			...
			if($dateFlag) {
				...
				if(customCompute($classes) && $classesID > 0) {
					if(customCompute($section) || $sectionID == 0) {
	...
	private function generateXML($data) {
		...
		if(customCompute($students)) {
		    ...
		    $attendancedate = date('d-m-Y',strtotime((string) $date));
