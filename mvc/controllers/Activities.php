...
public $load;
 ...
 public $data;
 public $uri;
 public $activitiescomment_m;
 public $activities_m;
 public $input;
 public $form_validation;
 public $upload;
 public $activitiesmedia_m;
 /*
| -----------------------------------------------------
...
*/
	...
	public function index() {
        ...
        if (($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1) && $_POST) {
            $id = htmlentities((string) escapeString($this->uri->segment(3)));
            if ((int)$id !== 0 && $_POST['comment']) {
    ...
    public function loadRecord(){
        ...
        $data = $this->activities_m->get_activities_pagination($this->input->post('limit'), $this->input->post('start'),$schoolyearID);
        $load_data = '';
        if($data->num_rows() > 0)
        {
            $this->data['activities'] =$data->result();
            $load_data = $this->load->view('activities/load_data', $this->data, true);
        }
	...
	public function add() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            $categoryID = htmlentities((string) escapeString($this->uri->segment(3)));
            if((int)$categoryID !== 0) {
                $schoolyearID = $this->session->userdata('defaultschoolyearID');
                ...
                if($_POST !== []) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        if ($this->input->post("time_to") != "0:00"){
                            $array["time_to"] = date('H:i:s',strtotime((string) $this->input->post("time_to")));
                        }
                        if ($this->input->post("time_from") != "0:00"){
                            $array["time_from"] = date('H:i:s',strtotime((string) $this->input->post("time_from")));
                        }
                        if ($this->input->post("time_at") != "0:00"){
                            $array["time_at"] = date('H:i:s',strtotime((string) $this->input->post("time_at")));
                        }
