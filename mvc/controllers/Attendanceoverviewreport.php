 ...
 public function __construct() {
		...
		if(permissionChecker('attendancereport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->data['getHolidays'] = explode('","', (string) $this->getHolidaysSession());
	...
	public function pdf() {
		if(permissionChecker('attendanceoverviewreport')) {		
			$usertype   = htmlentities((string) escapeString($this->uri->segment(3)));
			...
			$subjectID = 0;
			if($this->data["siteinfos"]->attendance == 'subject') {
				$subjectID  = htmlentities((string) escapeString($this->uri->segment(6)));
				$userID     = htmlentities((string) escapeString($this->uri->segment(7)));
				$monthID    = date('d-m-Y',(int)htmlentities((string) escapeString($this->uri->segment(8))));
				if($usertype == 1) {
					$flag = FALSE;
				}
			} else{
				$userID     = htmlentities((string) escapeString($this->uri->segment(6)));
				$monthID    = date('d-m-Y',(int)htmlentities((string) escapeString($this->uri->segment(7))));
			}
	...
	private function generateXML($data) {
		...
		if(customCompute($users)) {
			$sheet = $this->phpspreadsheet->spreadsheet->getActiveSheet();
			$monthday = explode('-', (string) $monthID);
          ...
          if(isset($attendances[$user->srstudentID])) {
                      ...
                      $bodys[$i][] = "H";
                      $holidayCount++;
                  } elseif (in_array($currentDate, $getWeekendDays)) {
                      $bodys[$i][] = "W";
                      $weekendayCount++;
                  } elseif(in_array($currentDate, $userleaveapplications)) {
                         $bodys[$i][] = 'LA';
                         $leavedayCount++;
                     } elseif ($attendances[$user->srstudentID]->$atten == NULL) {
                      $bodys[$i][] = "N/A";
                  } elseif ($attendances[$user->srstudentID]->$atten == NULL) {
      ...
      } elseif (isset($attendances[$user->srstudentID])) {
          for($k=1; $k <= $getDayOfMonth; $k++) { 
	                            	$atten = "a".$k; 
	                                $currentDate = sprintf("%02d", $k).'-'.$monthID;
	                                if (in_array($currentDate, $getHolidays)) {
	                	...
	                	if(isset($attendances[$user->teacherID])) {
                            for($k=1; $k <= $getDayOfMonth; $k++) { 
                                ...
                                if (in_array($currentDate, $getHolidays)) {
                                    ...
                                    $bodys[$i][] = 'LA';
                                    $leavedayCount++;
                                } elseif ($attendances[$user->teacherID]->$atten == NULL) {
                                    $bodys[$i][] = "N/A";
	                	...
	                	if(isset($attendances[$user->userID])) {
                            for($k=1; $k <= $getDayOfMonth; $k++) { 
                                ...
                                if (in_array($currentDate, $getHolidays)) {
                                    ...
                                    $bodys[$i][] = 'LA';
                                    $leavedayCount++;
                                } elseif ($attendances[$user->userID]->$atten == NULL) {
                                    $bodys[$i][] = 'N/A';
