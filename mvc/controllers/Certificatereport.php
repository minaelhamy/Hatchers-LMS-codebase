...
*/
	...
	public function generate_certificate() {
		...
		if(permissionChecker('certificatereport')) {
			if((int)$userID && (int)$usertypeID && (int)$templateID && (int)$schoolyearID && (int)$classID) {
				...
				if(customCompute($student) && customCompute($usertype) && customCompute($template) && customCompute($schoolyear)) {
					...
					$tagArray['[class]'] 		= customCompute($tagClasses) ? $tagClasses->classes : '';
					$tagArray['[section]'] 		= customCompute($tagSection) ? $tagSection->section : '';
					$tagArray['[group]'] 		= customCompute($tagGroup) ? $tagGroup->group : '';
					$tagArray['[optional_subject]'] = customCompute($tagSubject) ? $tagSubject->subject : '';
					...
					$this->data['template'] = $this->tagConvertForTemplate($userTags, $template->template, $tagArray, 3);

					$this->data['top_heading_title'] = $this->tagConvertForTemplate($userTags, $template->top_heading_title, $tagArray, 3, FALSE);

					$this->data['top_heading_left'] = $this->tagConvertForTemplate($userTags, $template->top_heading_left, $tagArray, 3, FALSE);

					$this->data['top_heading_middle'] = $this->tagConvertForTemplate($userTags, $template->top_heading_middle, $tagArray, 3, FALSE);

					$this->data['top_heading_right'] = $this->tagConvertForTemplate($userTags, $template->top_heading_right, $tagArray, 3, FALSE);

					$this->data['main_middle_text'] = $this->tagConvertForTemplate($userTags, $template->main_middle_text, $tagArray, 3, FALSE);

					$this->data['footer_left_text'] = $this->tagConvertForTemplate($userTags, $template->footer_left_text, $tagArray, 3, FALSE);

					$this->data['footer_middle_text'] = $this->tagConvertForTemplate($userTags, $template->footer_middle_text, $tagArray, 3, FALSE);

					$this->data['footer_right_text'] = $this->tagConvertForTemplate($userTags, $template->footer_right_text, $tagArray, 3, FALSE);
	...
	private function tagConvertForTemplate($userTags, $message, $convertArray, $usertypeID=1, $design = TRUE) {
        if($message && $usertypeID == 3 && customCompute($userTags)) {
