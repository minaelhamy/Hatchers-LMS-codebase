...
*/
            ...
            if($_POST !== []) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if($this->input->post('categoryID')) {
                        ...
                        $this->data['categoryName'] = customCompute($leavecategory) ? $leavecategory->leavecategory : '';
                    }
    ...
    public function pdf() {
        if(permissionChecker('leaveapplicationreport')) {
            ...
            $categoryID = htmlentities((string) escapeString($this->uri->segment(7)));
            $statusID   = htmlentities((string) escapeString($this->uri->segment(8)));
            $fromdate   = htmlentities((string) escapeString($this->uri->segment(9)));
            $todate     = htmlentities((string) escapeString($this->uri->segment(10)));
    ...
    private function queryArray($posts) {
        ...
        if($fromdate && $todate) {
            $fromdate = date('Y-m-d',strtotime((string) $fromdate));
            $queryArray['fromdate'] = $fromdate." 00:00:00";
            $todate = date('Y-m-d',strtotime((string) $todate));
        ...
        if(permissionChecker('leaveapplicationreport')) {
            if($_POST !== []) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    echo json_encode($retArray);
                    exit;
                } elseif (((int)$usertypeID || $usertypeID == 0) && ((int)$classesID || $classesID == 0) && ((int)$sectionID || $sectionID == 0) && ((int)$userID || $userID == 0) && ((int)$categoryID || $categoryID == 0) && ((int)$statusID || $statusID == 0) && ((int)$fromdate || $fromdate == null) && ((int)$todate || $todate == null)) {
                    $qArray['usertypeID'] = $usertypeID;
                    $qArray['classesID'] = $classesID;
                    $qArray['sectionID'] = $sectionID;
                    $qArray['userID'] = $userID;
                    ...
                    if((int)$this->input->post('categoryID') !== 0) {
                        $leavecategory = $this->leavecategory_m->get_single_leavecategory(array('leavecategoryID'=>$this->input->post('categoryID')));
                        $this->data['categoryName'] = customCompute($leavecategory) ? $leavecategory->leavecategory : '';
                    }
                    if($this->input->post('statusID')) {    
