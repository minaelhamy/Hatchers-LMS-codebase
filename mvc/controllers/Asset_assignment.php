    ...
    */
        ...
        if($_POST !== []) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if($this->input->post('check_out_date')) {
                    $array["check_out_date"] = date("Y-m-d", strtotime((string) $this->input->post("check_out_date")));
                }
                if($this->input->post('due_date')) {
                    $array["due_date"] 		= date("Y-m-d", strtotime((string) $this->input->post("due_date")));
                }
                if($this->input->post('check_in_date')) {
                    $array["check_in_date"] = date("Y-m-d", strtotime((string) $this->input->post("check_in_date")));
                }
            ...
            if($this->data['asset_assignment']) {
                ...
                if($_POST !== []) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $array["due_date"] = $this->input->post('due_date') ? date("Y-m-d", strtotime((string) $this->input->post("due_date"))) : Null;
                        if($this->input->post('check_in_date')) {
                            $array["check_in_date"] = date("Y-m-d", strtotime((string) $this->input->post("check_in_date")));
                        } else {
    ...
    public function view() {
        ...
        if((int)$id !== 0) {
            ...
            if(customCompute($this->data['asset_assignment'])) {
                $usertypeID = $this->data['asset_assignment']->usertypeID;

                if($usertypeID == 3) {
                    $student = $this->student_m->get_single_student(array('studentID' => $this->data['asset_assignment']->check_out_to));

                    $this->data['user'] = customCompute($student) ? $this->allUsersArrayObject($usertypeID, $student->classesID) : array();
                } else {
    ...
    public function valid_quantity($quantity) {
        ...
        if($quantity && $assetID) {
            ...
            if(customCompute($asset_purchases)) {
                ...
                if (customCompute($assigned_asset) && $assigned_asset->assetID == $assetID) {
