...
*/
	...
	public function add() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			$this->data['categorys'] = $hostelID > 0 ? $this->category_m->get_order_by_category(array("hostelID" => $hostelID)) : [];
	...
	public function print_preview() {
        ...
        $this->data['studentgroups'] = pluck($this->studentgroup_m->get_studentgroup(), 'group', 'studentgroupID');
        $this->data['optionalSubjects'] = pluck($this->subject_m->general_get_order_by_subject(array('type' => 0)), 'subject', 'subjectID');
		if(permissionChecker('hmember_view') || (($this->session->userdata('usertypeID') == 3) && permissionChecker('hmember') && ($this->session->userdata('loginuserID') == htmlentities((string) escapeString($this->uri->segment(3)))))) {
	...
	public function unique_gender() {
		...
		if((int)$id !== 0) {
			if($this->input->post("hostelID") == 0) {
				...
				if($hostel) {
					$gender = "";
					$gender = $student->sex == "Male" ? "Boys" : "Girls";
