...
*/
	...
	public function print_preview() {
		if(permissionChecker('tmember_view') || (($this->session->userdata('usertypeID') == 3) && permissionChecker('tmember') && ($this->session->userdata('loginuserID') == htmlentities((string) escapeString($this->uri->segment(3)))))) {
			$id = htmlentities((string) escapeString($this->uri->segment(3)));
			$url = htmlentities((string) escapeString($this->uri->segment(4)));
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			if((int)$id && (int)$url) {
