...
*/
	...
	public function index() {
		...
		$myProfile = false;
		if(isset($usertype[$this->session->userdata('usertypeID')]) && !permissionChecker('user_view')) {
			$myProfile = true;
		}
	...
	public function delete() {
		...
		if((int)$id !== 0) {
			$this->data['user'] = $this->user_m->get_single_user(array('userID' => $id));
			if($this->data['user']) {
				if(config_item('demo') == FALSE && ($this->data['user']->photo != 'default.png' && $this->data['user']->photo != 'defualt.png') && file_exists(FCPATH.'uploads/images/'.$this->data['user']->photo)) {
	...
	private function basicInfo($users) {
		$this->data['profile'] = customCompute($users) ? $users : [];
	}
	...
	public function download_document() {
		...
		$usertypeID 	= htmlentities((string) escapeString($this->uri->segment(5)));
	...
	public function print_preview() {
		...
		if(permissionChecker('user_view') || ((isset($usertype[$this->session->userdata('usertypeID')])) && permissionChecker('user') && ($this->session->userdata('loginuserID') == htmlentities((string) escapeString($this->uri->segment(3)))))) {
