 ...
 public function __construct() {
		...
		if(permissionChecker('salaryreport')) {
			if($_POST !== []) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
                    ...
                    $this->data['month']        = empty($this->input->post('month')) ? '0' : strtotime($month);
    ...
    public function pdf() {
		if(permissionChecker('salaryreport')) {
            ...
            $userID     = htmlentities((string) escapeString($this->uri->segment(4)));
            $fromdate   = htmlentities((string) escapeString($this->uri->segment(5)));
            $todate     = htmlentities((string) escapeString($this->uri->segment(6)));
            $month      = htmlentities((string) escapeString($this->uri->segment(7)));
	...
	private function generateXML($array) {
        ...
        if(customCompute($salarys)) {
            ...
            foreach($salarys as $salary) {
                $bodys[$i][] = $i+1;
                $bodys[$i][] = date('d M Y',strtotime((string) $salary->create_date));
		...
		if(permissionChecker('salaryreport')) {
			if($_POST !== []) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
                    ...
                    $postArray['userID'] = $userID;
                    if($month != '') {
                        $monthDate = date('d-m-Y',strtotime((string) $month));
                        $monthArray = explode('-',$monthDate);
                        $postArray['month'] = $monthArray[1].'-'.$monthArray[2];
                    }

                    if($fromdate != '' && $todate != '') {
                        $postArray['fromdate']  = date('d-m-Y',strtotime((string) $fromdate));
                        $postArray['todate']    = date('d-m-Y',strtotime((string) $todate));
                    }

                    $this->data['usertypeID']= $usertypeID;
                    $this->data['userID']    = $userID;
                    $this->data['month']     = strtotime((string) $month);
                    $this->data['fromdate']  = strtotime((string) $fromdate);
                    $this->data['todate']    = strtotime((string) $todate);
