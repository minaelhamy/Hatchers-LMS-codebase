...
*/
    ...
    private function _browseFileUpload($file)
    {
        ...
        if (isset($file['file'])) {
            ...
            $endArray = explode('.', (string) $file['file']['name']);
            ...
            if (in_array($fileExt, $extensions)) {
                if ($fileSize <= $maxFileSize) {
                    ...
                    $returnArray['version']      = str_replace('.zip', '', (string) $fileName);
    ...
    private function _postData()
    {
        ...
        if (customCompute($updates)) {
            $postDatas = [
                ...
                'licensekey'     => customCompute($this->data['siteinfos']) ? $this->data['siteinfos']->license_code : '',
                'domainname'     => base_url(),
    ...
    private function _fileDownload($result)
    {
        ...
        $guzzle = new Guzzle();
        $response = $guzzle->request('', $this->_updateFileUrl);
        $header      = explode(';', (string) $response->getHeader('Content-Type')[0]);
        $contentType = $header[0];
        if ($contentType == 'application/json') {
    ...
    private function _smartCopy($source, $dest, $options = ['folderPermission' => 0777, 'filePermission' => 0777])
    {
        ...
        if (is_file($source)) {
            if ($dest[strlen((string) $dest) - 1] == '/') {
                if (!file_exists($dest)) {
                    cmfcDirectory::makeAll($dest, $options['folderPermission'], true);
                }
                $__dest = $dest . "/" . basename((string) $source);
            } else {
                $__dest = $dest;
            }
            ...
            if ($dest[strlen((string) $dest) - 1] == '/') {
                if ($source[strlen((string) $source) - 1] == '/') {
                    //Copy only contents
                } else {
                    //Change parent itself and its contents
                    $dest .= basename((string) $source);
                    @mkdir($dest);
                    @chmod($dest, $options['filePermission']);
                }
            } elseif ($source[strlen((string) $source) - 1] == '/') {
            ...
            while ($file = readdir($dirHandle)) {
                if ($file != "." && $file != "..") {
                    $__dest = is_dir($source . "/" . $file) ? $dest . "/" . $file : $dest . "/" . $file;
    ...
    private function _databaseUpdate()
    {
        ...
        if (file_exists($this->_downloadExtractPath . 'inilabs.json')) {
            ...
            if ($inilabsString !== '' && $inilabsString !== false) {
                $inilabsJsonArray = json_decode($inilabsString, true);
                if (isset($inilabsJsonArray['database']['status']) && strtolower((string) $inilabsJsonArray['database']['status']) != 'no' && (isset($inilabsJsonArray['filename']) && !empty($inilabsJsonArray['filename']))) {
    ...
    private function _deleteZipAndFile($filePathAndName)
    {
        ...
        try {
            ...
            if (file_exists($filePathAndName)) {
                ...
                $filePathAndName = str_replace(".zip", "", (string) $filePathAndName);
    ...
    private function _successProvider($postDatas)
    {
        ...
        $guzzle = new Guzzle();
        $response = $guzzle->request($postDataStrings, $this->_successUrl);
        $header      = explode(';', (string) $response->getHeader('Content-Type')[0]);
        $contentType = $header[0];
        if ($contentType == 'application/json') {
            $contents = $response->getBody()->getContents();
