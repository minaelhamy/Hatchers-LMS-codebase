...
*/
							...
							if($_POST !== []) {
								...
								if ($this->form_validation->run() == FALSE) {
									...
									if(customCompute($maininvoice)) {
										...
										$maininvoicestatus = $amountcalculation <= 0 ? 2 : 1;

										$invoicepaidstatus = $this->input->post('amount') >= $duecalculation ? 2 : 1;
	...
	public function delete() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
			...
			if((int)$id !== 0) {
				...
				if(customCompute($singlePayment)) {
					if(($singlePayment->paymenttype != "Paypal") && ($singlePayment->paymenttype != 'Stripe') && ($singlePayment->paymenttype != 'PayUmoney') && ($singlePayment->paymenttype != 'Voguepay')) {
						...
						if(customCompute($singleInvoice)) {
           ...
           $invoicePaidStatus = 1;
       } elseif ($extraPaymentWeaverAmount > 0) {
           $invoicePaidStatus = 1;
       }
           ...
           $mainInvoiceStatus = 1;
       } elseif (($grandTotalPaymentAmount - $singlePayment->paymentamount) == 0) {
           $mainInvoiceStatus = 0;
       } else {
									$mainInvoiceStatus = 1;
								}
							...
							if($globalID) {
								...
								if(!customCompute($payments) && !customCompute($weaverFines)) {
