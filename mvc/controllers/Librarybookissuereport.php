...
class Librarybookissuereport extends Admin_Controller{
 ...
 public function __construct() {
        ...
        if(permissionChecker('librarybookissuereport')) {
			if($_POST !== []) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
					...
					$this->data['fromdate']  = empty($this->input->post('fromdate')) ? '0' : strtotime((string) $this->input->post('fromdate'));
					$this->data['todate']    = empty($this->input->post('todate')) ? '0' : strtotime((string) $this->input->post('todate'));
	...
	private function generateXML($data) {
		extract($data);
		if(customCompute($getLibrarybookissueReports)) {
			$sheet = $this->phpspreadsheet->spreadsheet->getActiveSheet();
			$maxColumnCount = 9;
       ...
       $sheet->setCellValue($headerColumn.$row, $sectionName);
	        ...
	        foreach($getLibrarybookissueReports as $libraryBooksReport) {
	        	...
	        	if($typeID == 0) {
	                if(isset($libraryBooksReport->issue_date)) {
	                    $libraryBooksReportArray[$i][] = date('d M Y', strtotime((string) $libraryBooksReport->issue_date));
	                } else {
	                    $libraryBooksReportArray[$i][] = '';
	                }

	                if(isset($libraryBooksReport->due_date)) {
	                    $libraryBooksReportArray[$i][] = date('d M Y', strtotime((string) $libraryBooksReport->due_date));
	                } else {
	                    $libraryBooksReportArray[$i][] = '';
	                }

	                if(isset($libraryBooksReport->return_date)) {
	                    $libraryBooksReportArray[$i][] = date('d M Y', strtotime((string) $libraryBooksReport->return_date));
	                } else {
	                    $libraryBooksReportArray[$i][] = '';
	                }
	            } elseif($typeID == 1) {
            ...
            //Make Here Xml Body
	        $row  = 3;
	        if(customCompute($libraryBooksReportArray)) {
