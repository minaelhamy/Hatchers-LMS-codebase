...
*/
	...
	public function pdf() {
		if(permissionChecker('studentfinereport')) {
			$fromdate = htmlentities((string) escapeString($this->uri->segment(3)));
			$todate  = htmlentities((string) escapeString($this->uri->segment(4)));
	...
	private function generateXML($data) {
		...
		if(customCompute($studentfines)) {
			...
			$fromdateValue .= " : ";
			$fromdateValue .= date('d M Y',strtotime((string) $fromdate));

			$todateValue  = $this->lang->line('studentfinereport_todate');
			$todateValue .= " : ";
			$todateValue .= date('d M Y',strtotime((string) $todate));
			...
			foreach($studentfines as $studentfine) {
				$bodys[$i]['slno'] = $j;
				$bodys[$i]['date'] = date('d M Y',strtotime((string) $studentfine->paymentdate));
