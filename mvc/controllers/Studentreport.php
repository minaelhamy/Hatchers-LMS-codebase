...
*/
	...
	public function pdf() {
		if(permissionChecker('studentreport')) {
			...
			$transport      = htmlentities((string) escapeString($this->uri->segment(6)));
			$hostel         = htmlentities((string) escapeString($this->uri->segment(7))); 
			$gender         = htmlentities((string) escapeString($this->uri->segment(8))); 
			$birthdaydate   = htmlentities((string) escapeString($this->uri->segment(9)));
			$classesID      = htmlentities((string) escapeString($this->uri->segment(10)));
			$sectionID      = htmlentities((string) escapeString($this->uri->segment(11)));
		...
		if(permissionChecker('studentreport')) {
        	if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if((string) $reportfor && ((int) $blood || $blood >= 0) && ((int) $country || $country >= 0) && ((int) $transport || $transport >= 0) && ((int) $gender || $gender >= 0) && ((int) $birthdaydate || $birthdaydate >= 0) && ((int) $classesID || $classesID >= 0) && ((int) $sectionID || $sectionID == NULL || $sectionID >= 0)) {
						if(in_array(strtolower((string) $reportfor), $reportforArray)) {
							if($bloodID <= 7) {
								$allCountry = $this->getAllCountry();
								$allCountry[0] = 'Select One';
