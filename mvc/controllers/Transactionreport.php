 ...
 public function __construct() {
		...
		if(permissionChecker('transactionreport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = [];
					$array['fromdate']     = date('Y-m-d',strtotime((string) $this->input->post('fromdate')));
					$array['todate']       = date('Y-m-d',strtotime((string) $this->input->post('todate')));
		...
		if($xmloption == 1 ) {
			if(customCompute($get_payments)) {
				...
				foreach ($get_payments as $get_payment) {
					if(isset($weaverandfine[$get_payment->paymentID]) && (($weaverandfine[$get_payment->paymentID]->weaver != '') || ($weaverandfine[$get_payment->paymentID]->fine != '')) || $get_payment->paymentamount != '') {
						...
						$bodys[$i][] = $i+1;
						$bodys[$i][] = date('d M Y',strtotime((string) $get_payment->paymentdate));
                        ...
                        if(isset($students[$get_payment->studentID]) && isset($classes[$students[$get_payment->studentID]->srclassesID])) {
                            $bodys[$i][] = $classes[$students[$get_payment->studentID]->srclassesID];  
                        }
                        
                        if(isset($students[$get_payment->studentID]) && isset($sections[$students[$get_payment->studentID]->srsectionID])) {
			...
			if(customCompute($incomes)) {
				...
				foreach($incomes as $income) {
					$bodys[$i][] = $income->name;
					$bodys[$i][] = date('d M Y',strtotime((string) $income->date));
			...
			if(customCompute($expenses)) {
				...
				foreach($expenses as $expense) {
					...
					$bodys[$i][] = date('d M Y',strtotime((string) $expense->date));
	...
	public function unique_date_new() {
		...
		if($fromdate != '' && $todate != '') {
			...
			if((strtotime($fromdate) < strtotime((string) $startingdate)) || (strtotime($fromdate) > strtotime((string) $endingdate))) {
				$this->form_validation->set_message("unique_date_new", "The from date is invalid .");
			    return FALSE;
			}
			if((strtotime($todate) < strtotime((string) $startingdate)) || (strtotime($todate) > strtotime((string) $endingdate))) {
