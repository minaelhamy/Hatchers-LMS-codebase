...
class Productpurchasereport extends Admin_Controller {
 ...
 public $productsupplier_m;
 public $productwarehouse_m;
 ...
 public function __construct() {
		...
		if(permissionChecker('productpurchasereport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    $this->data['reference_no'] = empty($this->input->post('reference_no')) ? '0' : $this->input->post('reference_no');
                    ...
                    if(customCompute($productpurchases)) {
                        foreach ($productpurchases as $productpurchase) {
                            if(isset($productpurchaseArray[$productpurchase->productpurchaseID])) {
                                ...
                                $productpurchaseArray[$productpurchase->productpurchaseID]['date'] = date('d M Y',strtotime((string) $productpurchase->productpurchasedate));
                                $productpurchaseArray[$productpurchase->productpurchaseID]['total'] = ($productpurchase->productpurchaseunitprice * $productpurchase->productpurchasequantity);
	...
	public function pdf() {
		if(permissionChecker('productpurchasereport')) {
            $productsupplierID  = htmlentities((string) escapeString($this->uri->segment(3)));
            $productwarehouseID = htmlentities((string) escapeString($this->uri->segment(4)));
            $reference_no       = htmlentities((string) escapeString($this->uri->segment(5)));
            $statusID = htmlentities((string) escapeString($this->uri->segment(6)));
            $fromdate = htmlentities((string) escapeString($this->uri->segment(7)));
            $todate   = htmlentities((string) escapeString($this->uri->segment(8)));
		...
		if(permissionChecker('productpurchasereport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    $this->data['reference_no'] = empty($this->input->post('reference_no')) ? '0' : $this->input->post('reference_no');
                    $this->data['statusID'] = $this->input->post('statusID');
                    $this->data['fromdate'] = strtotime((string) $this->input->post('fromdate'));
                    $this->data['todate'] = strtotime((string) $this->input->post('todate'));
    ...
    public function unique_date() {
        ...
        if($fromdate != '' && $todate != '') {
            ...
            if((strtotime((string) $fromdate) < strtotime((string) $startingdate)) || (strtotime((string) $fromdate) > strtotime((string) $endingdate))) {
                $this->form_validation->set_message("unique_date", "The from date are invalid .");
                return FALSE;
            }

            if((strtotime((string) $todate) < strtotime((string) $startingdate)) || (strtotime((string) $todate) > strtotime((string) $endingdate))) {
