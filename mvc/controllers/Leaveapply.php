...
class Leaveapply extends Admin_Controller
{
    ...
    */
    ...
    public function attachmentUpload() {
        ...
        } elseif (customCompute($leaveapplication)) {
        ...
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
            ...
            foreach ($this->data['leavecategories'] as $item) {
                if (array_key_exists($item->leavecategoryID, $result)) {
                    $item->leaveassignday -= $result[$item->leavecategoryID];
                }
            }

            foreach ($this->data['usertypes'] as $usertypeKey => $usertype) {
                if (customCompute($usertype) && $usertype->usertypeID == 3 || $usertype->usertypeID == 4) {
            ...
            if ($_POST !== []) { 
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $explode            = explode('-', (string) $this->input->post("leave_schedule"));
                    $array["from_date"] = date("Y-m-d", strtotime(trim($explode[0])));
                    $array["to_date"] = date("Y-m-d", strtotime(trim($explode[1])));
                    $leavedaysCount = $this->leavedayscustomCompute(trim($explode[0]), trim($explode[1]));
                ...
                if (customCompute($leaveapplication)) {
                    if($leaveapplication->status == NULL) {
                        if($leaveapplication->create_userID == $this->session->userdata('loginuserID') && $leaveapplication->create_usertypeID == $this->session->userdata('usertypeID')) {
                            if (config_item('demo') == FALSE && $leaveapplication->attachment != null && file_exists(FCPATH . 'uploads/leaveapplications/' . $leaveapplication->attachment)) {
    ...
    public function usercall() {
        ...
        if((int)$usertypeID !== 0) {
            ...
            if($usertypeID == 1) {
                ...
                if(customCompute($this->data['users'])) {
        ...
        if ((int)$id !== 0) {
            ...
            if(customCompute($this->data['leaveapply'])) {
                if(($this->data['leaveapply']->create_userID == $this->session->userdata('loginuserID')) && ($this->data['leaveapply']->create_usertypeID == $this->session->userdata('usertypeID'))) {
                    ...
                    $this->data['leaveapply']->category = customCompute($leavecategory) ? $leavecategory->leavecategory : '';
                    ...
                    $this->data['daysArray'] = $this->leavedayscustomCompute($this->data['leaveapply']->from_date, $this->data['leaveapply']->to_date);
