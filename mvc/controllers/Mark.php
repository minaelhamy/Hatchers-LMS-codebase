...
*/
	        ...
	        if ($this->form_validation->run() == FALSE) {
				...
				if(customCompute($inputs)) {
					foreach ($inputs as $key => $value) {
						$data = explode('-', (string) $value['mark']);
	...
	public function print_preview() {
		if(permissionChecker('mark_view') || (($this->session->userdata('usertypeID') == 3) && permissionChecker('mark') && ($this->session->userdata('loginuserID') == htmlentities((string) escapeString($this->uri->segment(3)))))) {
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
	...
	public function unique_inputs() {
     ...
     } elseif (is_string($markValue) && $markValue != '') {
