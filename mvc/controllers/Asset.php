    ...
    */
    ...
    public function delete() {
        ...
        if((int)$id !== 0) {
            $this->data['asset'] = $this->asset_m->get_single_asset(array('assetID' => $id));
            if($this->data['asset']) {
                if(config_item('demo') == FALSE && $this->data['asset']->attachment && file_exists(FCPATH.'uploads/attach/'.$this->data['asset']->attachment)) {
    ...
    public function print_preview() {
        if(permissionChecker('asset_view')) {
            $assetID = htmlentities((string) escapeString($this->uri->segment(3)));
            if((int)$assetID !== 0) {
