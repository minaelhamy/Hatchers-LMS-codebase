...
*/
	...
	public function getpages() {
		if($_POST !== []) {
			...
			$pageArrays = [];
			if(customCompute($pages)) {
				foreach ($pages as $page) {
					$expPage = explode('-', (string) $page);
					$pageID = $expPage[1];
	...
	public function getlinks() {
		if($_POST !== []) {
			...
			if(!empty($urlLinkField) && !empty($urlLinkText)) {
				if (strpos((string) $urlLinkField, 'http://') === false && strpos((string) $urlLinkField, 'https://') === false && $urlLinkField != '#') {
	...
	public function managelocation() {
		...
		if ($_POST !== [] && customCompute($this->fmenu_m->get_fmenu())) {
	...
	public function savemenu() {
		...
		if($_POST !== []) {
			if(customCompute($this->fmenu_m->get_fmenu())) {
				$status = FALSE;
	...
	private function validation_save_menu($elements) {
		...
		foreach ($elements as $key => $element) {
			if(strlen((string) $element['menu_label']) > 253 ) {
	...
	private function validation_xss_clen($elements) {
		...
		if(customCompute($elements)) {
			foreach ($elements as $key => $element) {
				...
				$element['menu_link'] = empty($element['menu_link']) ? '#' : xssRemove($element['menu_link']);
				$retArray[] = $element;
			}
		}
		return $retArray;
	...
	public function createnewmenu() {
		...
		if($_POST !== []) {
			...
			if(!empty($menuName)) {
				...
				if(strlen((string) $menuName) <= 128) {
					$fmenu = $this->fmenu_m->get_fmenu();
					if(customCompute($fmenu)) {
