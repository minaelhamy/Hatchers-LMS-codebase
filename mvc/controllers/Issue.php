...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			...
			$ulID = htmlentities((string) escapeString($this->uri->segment(3)));
			$lID = htmlentities((string) escapeString($this->input->post("lid")));
			if($lID != "" || $lID !== '') {
			...
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = array(
						...
						"due_date" => date("Y-m-d", strtotime((string) $this->input->post("due_date"))),
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			$id = htmlentities((string) escapeString($this->uri->segment(3)));
			$lID = htmlentities((string) escapeString($this->uri->segment(4)));
			$this->data['books'] = $this->book_m->get_book();
			if((int)$id && $lID) {
			...
			if(customCompute($this->data['book'])) {
				...
				if(customCompute($lmember)) {
					...
					if(customCompute($this->data['student'])) {
						...
						} elseif ($this->data['book']->return_date == NULL) {
          $this->data["subview"] = "issue/view";
          $this->load->view('_layout_main', $this->data);
      } else {
								$this->data["subview"] = "error";
	...
	public function wrong_date() {
		$due_date = strtotime(date("Y-m-d", strtotime((string) $this->input->post("due_date"))));
