...
*/
			...
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$array = array(
						...
						"edate" => date("Y-m-d", strtotime((string) $this->input->post("date"))),
	...
	public function check_duplicate_subject() {
		if($_POST !== []) {
			$examscheduleID = htmlentities((string) escapeString($this->uri->segment(3)));
			$qArray = [];
