...
class Searchpaymentfeesreport extends Admin_Controller {
 ...
 public function __construct() {
		...
		if(permissionChecker('searchpaymentfeesreport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
                    ...
                    $count = strlen((string) $gspaymentIDstring);
                    $gspayment =  substr((string) $gspaymentIDstring,  -$count, 6);
                    $gspaymentID =  substr((string) $gspaymentIDstring, 6,$count);
                    ...
                    if((int)$gspaymentID && (($gspayment == 'INV-G-') || ($gspayment == 'inv-g-'))) {
                        ...
                        if(customCompute($this->data['singlepayments'])) {
	...
	public function pdf() {
		if(permissionChecker('searchpaymentfeesreport')) {
			$urlgspaymentID = htmlentities((string) escapeString($this->uri->segment(3)));
    ...
    public function check_invoiceID() {
    	$gspaymentID = trim((string) $this->input->post('gspaymentID'));
    	if($gspaymentID != '') {
