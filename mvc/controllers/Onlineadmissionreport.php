...
*/
		...
		if(permissionChecker('onlineadmissionreport')) {
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->data['admissionID'] = empty($this->input->post('admissionID')) ? '0' : $this->input->post('admissionID');
	...
	public function pdf() {
		if(permissionChecker('onlineadmissionreport')) {
			$schoolyearID = htmlentities((string) escapeString($this->uri->segment(3)));
			$classesID    = htmlentities((string) escapeString($this->uri->segment(4)));
			$status       = htmlentities((string) escapeString($this->uri->segment(5)));
			$phone        = htmlentities((string) escapeString($this->uri->segment(6)));
			$admissionID  = htmlentities((string) escapeString($this->uri->segment(7)));
