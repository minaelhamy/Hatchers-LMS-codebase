...
*/
    ...
    protected function rules()
    {
        $rules = [
            ...
            [
                'field' => 'license_code',
                'label' => $this->lang->line("license_code"),
                'rules' => 'trim|required|xss_clean'
            ]
        ];
    ...
    {
        $themesID  = htmlentities((string) escapeString($this->input->post('id')));
        $themeName = 'default';
        if ( (int) $themesID !== 0 ) {
            $theme = $this->themes_m->get_single_themes([ 'themesID' => $themesID ]);
            if ( customCompute($theme) ) {
                $themeName = strtolower(str_replace(' ', '', (string) $theme->themename));
            }
        }
