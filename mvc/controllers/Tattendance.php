...
*/
	...
	public function index() {
		$myProfile = false;
		if($this->session->userdata('usertypeID') == 2 && !permissionChecker('tattendance_view')) {
			$myProfile = true;
		}
			...
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if(customCompute($teachers)) {
						...
						if(customCompute($insertArray)) {
							$this->tattendance_m->insert_batch_tattendance($insertArray);
						}

						$this->data['dateinfo']['day'] = date('l', strtotime((string) $date));
						$this->data['dateinfo']['date'] = date('jS F Y', strtotime((string) $date));
