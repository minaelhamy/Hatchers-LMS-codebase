...
class Childcare extends Admin_Controller {
 ...
 function __construct() {
			...
			if($_POST !== []) {
				...
				if ($this->form_validation->run() == FALSE) { 
	                ...
	                if($this->input->post('userID')) {
	                	...
	                	$data['parentID'] = customCompute($student) ? $student->parentID : 0;
	                }
	...
	public function edit() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id !== 0) {
				...
				if(customCompute($this->data['careinfo'])) {
			        $this->data['parents'] = $this->parents_m->get_parents();
			        $this->data['classes'] = $this->classes_m->get_classes();
	...
	public function delete() {
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1)) {
			...
			if((int)$id !== 0) {
				...
				if(customCompute($this->data['childcare'])) {
	...
	public function unique_date($date) {
		...
		if($date != '') {
			if((strtotime((string) $date) < strtotime((string) $startingdate)) || (strtotime((string) $date) > strtotime((string) $endingdate))) {
