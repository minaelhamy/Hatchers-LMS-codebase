...
class MY_Config Extends CI_Config {
 ...
 public $autoload_path		= ''; // Set in the constructor below
     ...
     * @var string
     */
    public $purchase_path      = ''; // Set in the constructor below
    ...
    function __construct()
    {
        ...
        if ( defined('ENVIRONMENT') ) {
            switch ( ENVIRONMENT ) {
                case 'development':
                    $this->database_path = APPPATH . 'config/development/database' . EXT;
                    break;
                case 'testing':
                case 'production':
                    $this->database_path = APPPATH . 'config/production/database' . EXT;
                    break;
                default:
        ...
        $this->purchase_path = APPPATH . 'config/purchase.php';
	...
	{
		if ( ! is_array($config_array) && customCompute($config_array) == 0)
		{
			return FALSE;
		}
	...
	function db_config_update($dbconfig = array(), $remove_values = array())
	{
		...
		if (customCompute($dbconfig) > 0)
		{
			foreach ($dbconfig as $key => $val)
			{
		        ...
		        $config_file = preg_replace($pattern, $replace, (string) $config_file);
			}
		}
